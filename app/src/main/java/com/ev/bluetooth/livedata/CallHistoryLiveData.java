...
package com.ev.bluetooth.livedata;
...
import com.ev.bluetooth.phonebook.common.AsyncQueryLiveData;
import com.ev.bluetooth.phonebook.common.PhoneCallLog;
import com.ev.bluetooth.phonebook.common.QueryParam;
...
public class CallHistoryLiveData extends AsyncQueryLiveData<List<PhoneCallLog>> {
    private final String TAG=CallHistoryLiveData.class.getSimpleName();
    /**
    ...
    public static CallHistoryLiveData newInstance(Context context) {
        return newInstance(context, CallType.CALL_TYPE_ALL, DEFAULT_CALL_LOG_LIMIT);
    }
    ...
    public static CallHistoryLiveData newLastCallLiveData(Context context) {
        return newInstance(context, CallType.CALL_TYPE_ALL, 1);
    }
    ...
    protected List<PhoneCallLog> convertToEntity(Cursor cursor) {
        ...
        for(PhoneCallLog log:resultList){
            Log.i(TAG, "convertToEntity: =log.getLastCallEndTimestamp() "+log.getLastCallEndTimestamp());
            Log.i(TAG, "convertToEntity: =log.getPhoneLogId()"+log.getPhoneLogId());
            Log.i(TAG, "convertToEntity: =log.getPhoneNumberString()"+log.getPhoneNumberString());
          //  Log.i(TAG, "****getDataList: =log.getCallRecords()*******"+log.getCallRecords());

            Log.i(TAG, "convertToEntity: =log.getAllCallRecords()"+log.getAllCallRecords());

            for(PhoneCallLog.Record record:log.getAllCallRecords()){
                Log.i(TAG, "convertToEntity: record:=record.getCallEndTimestamp()"+record.getCallEndTimestamp());
                Log.i(TAG, "convertToEntity: record:=+record.getCallType()"+record.getCallType());
            }
